...
#
# This file was adapted from DensPart and is now part of HORTON-Part.
#
...
# --
"""Convert a legacy GPAW restart into a HORTON-Part periodic density archive.

This module is adapted from :mod:`denspart.adapters.gpaw` on DensPart's ``lisa``
branch at commit ``ce0d986``. GPAW is imported lazily so that it remains an
...
__all__ = ["assemble_density_arrays", "prepare_input"]


def _load_gpaw():
    """Import the optional GPAW interfaces needed by this adapter."""
    try:
        ...
        raise RuntimeError(
            "part-from-gpaw requires GPAW, which is intentionally not a "
            "HORTON-Part dependency. Run this command in a GPAW environment."
        ) from exc
    return restart, unpack_density, world


def prepare_input(atoms, calc):
    """Prepare HORTON-Part input from a GPAW run.
    ...
    """
    # ASE is a transitive dependency of GPAW, but remains optional for HORTON-Part.
    try:
        from ase.units import Bohr
    except ImportError as exc:
        raise RuntimeError("Preparing GPAW input requires ASE.") from exc
    ...
    uniform_data = get_uniform_grid_data(calc, cellvecs, atnums, Bohr)
    ...
    print("Converting to HORTON-Part arrays")
    density = assemble_density_arrays(uniform_data, atoms)
    density.update(
        {
            "atcoords": atcoords,
            "atnums": atnums,
...
def get_uniform_grid_data(calc, cellvecs, atnums, bohr):
    ...
    bohr
        The ASE conversion factor from bohr to angstrom.
    ...
    # TODO: the criterion is too strict.
...
def compute_augmentation_spheres(uniform_data, setups, atoms, atnums, atcoords):
    ...
    print("  ~~~~~~~  ~~~~~~~~~~~~~  ~~~~~~~~~~~~~  ~~~~~~~~~~~~~")
    print("     Atom  Rebuilt QCor       GPAW QCor          Error")
    print("  ~~~~~~~  ~~~~~~~~~~~~~  ~~~~~~~~~~~~~  ~~~~~~~~~~~~~")
    ...
    print(f"  Rebuilt total charge:  {nelec_pseudo + myqcors.sum():10.3e}")
    assert_allclose(qcors, myqcors, atol=1e-10)
    if sqcors is not None:
        print(f"  GPAW total spin:       {spin_pseudo + sqcors.sum():10.3e}")
        print(f"  Rebuilt total spin:    {spin_pseudo + mysqcors.sum():10.3e}")
...
def assemble_density_arrays(uniform_data, atoms):
    """Combine uniform and augmentation grids into HORTON-Part arrays.
    ...
    atoms
        List of dictionaries with atomic-grid data.

    Returns
    -------
    density
        Dictionary containing the data needed for periodic partitioning.
    ...
    if uniform_data["nspins"] == 2:
        ...
        result["spindensity"] = np.concatenate([gp.density for gp in spin_grid_parts])
...
def main(args=None):
    ...
    if world.size != 1:
        raise RuntimeError("part-from-gpaw must be run with one GPAW process.")
    print("Loading file", args.fn_gpw)
    atoms, calc = restart(args.fn_gpw, txt="/dev/null")
    if not getattr(calc, "old", False):
        raise RuntimeError(
            "This converter requires GPAW legacy mode because it reads PAW "
            "augmentation data from the legacy calculator internals."
        )
...
def parse_args(args):
    ...
    description = "Convert a legacy GPAW restart into HORTON-Part periodic input."
    parser = argparse.ArgumentParser(prog="part-from-gpaw", description=description)
