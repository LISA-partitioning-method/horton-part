...
def test_becke_nonlocal_lih_hf_321g():
    # load molecule data
    coords, nums, pnums, dens, points = load_molecule_npz(
        "li_h_3-21G_hf_g09_fchk_exp:1e-3:1e1:100:110.npz"
    )
    ...
    grid1 = BeckeMolGrid(
        coords, nums, pnums, (rgrid, 110), random_rotate=False, mode="only"
    )
    bp1 = BeckeWPart(coords, nums, pnums, grid1, dens)
    # check the grid points against stored points on which density is evaluated
    assert (abs(points - grid1.points) < 1.0e-6).all()

    grid2 = BeckeMolGrid(
        coords, nums, pnums, (rgrid, 110), random_rotate=False, mode="discard"
    )
    bp2 = BeckeWPart(coords, nums, pnums, grid2, dens, local=False)
    # check the grid points against stored points on which density is evaluated
    assert (abs(points - grid2.points) < 1.0e-6).all()
...
def check_becke_azirine(key, expected):
    # load molecule data
    coords, nums, pseudo_nums, dens, points = load_molecule_npz(
        "2h-azirine-%s-fchk-medium.npz" % key
    )
    # make grid
    grid = BeckeMolGrid(
        coords, nums, pseudo_nums, random_rotate=False, mode="only", agspec="medium"
    )
    # check the grid points against stored points on which density is evaluated
    assert (abs(points - grid.points) < 1.0e-6).all()
    # Becke partitioning
...
def test_becke_ch3_hf_sto3g():
    # load molecule data
    coords, nums, pnums, dens, spindens, points = load_molecule_npz(
        "ch3_hf_sto3g_fchk_medium.npz", True
    )
    grid = BeckeMolGrid(
        coords, nums, pnums, random_rotate=False, mode="only", agspec="medium"
    )
